...
function PANEL:Paint()
  if not IsValid(self.NewGameFrame) and not IsValid(self.AddonsFrame) and not IsValid(self.AchievementsFrame) and not IsValid(self.SavesFrame) then
...
function PANEL:ClosePopups(b)
  ...
  if not b and IsValid(self.MountedGamesList) then -- The ugly 'b' hack
    self.MountedGamesList:Remove()
  end
...
function PANEL:OpenLanguages(pnl)
  if IsValid(self.LanguageList) then
    self.LanguageList:Remove()
    return
  end
...
function PANEL:OpenMountedGamesList(pnl)
  if IsValid(self.MountedGamesList) then
    self.MountedGamesList:Remove()
    return
  end
  ...
  local function add(t)
    ...
    a:SetText(t.title)
    if not t.installed then a:SetText(t.title .. " ( not installed )") end
    if not t.owned then a:SetText(t.title .. " ( not owned )") end
    p:AddItem(a)
    a:SetChecked(t.mounted)
    a.OnChange = function(panel) engine.SetMounted(t.depot, a:GetChecked()) end
    if not t.owned or not t.installed then a:SetDisabled(true) end
  end
...
function PANEL:OpenGamemodesList(pnl)
  if IsValid(self.GamemodesList) then
    self.GamemodesList:Remove()
    return
  end
  ...
  for id, t in SortedPairsByMemberValue(engine.GetGamemodes(), "title") do
    if not t.menusystem then continue end
...
function PANEL:RefreshAddons()
  if not IsValid(self.AddonsFrame) then return end
...
timer.Simple(1, function() if gui.IsConsoleVisible() then gui.ShowConsole() end end)
--[[
