function Menu_InstallDButtonScrollProtection(pnl, depth, allowM2)
  ...
  function pnl:OnMousePressed(mc)
    ...
    if mc ~= MOUSE_LEFT and not allowM2 then self.Depressed = nil end
...
local function DrawToolTip(self, w, h)
  if not self:GetTooltip() then return end
  if not (self.Hovered or self.IsHovered and self:IsHovered()) then return end
  ...
  local minX = ScrW() / 4
  while screenX < minX do
    -- Probably could do with caching
...
function PANEL:GetMapCount()
  if not self.Category then return -1 end
  if not self.LastCache or self.LastCache < CurTime() or self.NewGameMenu.SearchText ~= self.SearchText then
...
function PANEL:Think()
  if self:GetConVar() and self.ThinkBlock < CurTime() then
    if GetConVarNumber(self:GetConVar()) > 0 then
...
function PANEL:Paint(w, h)
  ...
  if self:GetText() then draw.SimpleText(language.GetPhrase(self:GetText()), self:GetFont(), 5, h / 2, color_black, 0, 1) end
...
function PANEL:Init()
  ...
  self.TextEntry.OnValueChange = function(s) if self:GetConVar() then RunConsoleCommand(self:GetConVar(), s:GetText()) end end
...
function PANEL:Think()
  if not self.TextEntry:IsEditing() and self:GetConVar() and GetConVarNumber(self:GetConVar()) and GetConVarNumber(self:GetConVar()) ~= tonumber(self.TextEntry:GetText()) then self.TextEntry:SetText(GetConVarNumber(self:GetConVar())) end
end
...
function PANEL:Think()
  if not self.TextEntry:IsEditing() and self:GetConVar() and GetConVarString(self:GetConVar()) and GetConVarString(self:GetConVar()) ~= self.TextEntry:GetText() then self.TextEntry:SetText(GetConVarString(self:GetConVar())) end
end
...
function PANEL:Paint(w, h)
  ...
  surface.DrawRect(0, 0, w, h)
  self:DrawTextEntryText(self:GetTextColor(), self:GetHighlightColor(), self:GetCursorColor())
  if not self:GetText() or self:GetText():Trim():len() < 1 then draw.SimpleText(language.GetPhrase(self:GetPlaceholderText()), self:GetFont(), 5, h / 2, Vector(1, 1, 1) * 150, nil, 1) end
  DrawToolTip(self, w, h)
end
