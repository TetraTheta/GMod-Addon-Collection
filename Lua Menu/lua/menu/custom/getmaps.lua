...
local MapPatterns = {}
local MapNames = {}
MapNames["de_"] = "Counter-Strike" -- Bomb Defuse
MapNames["cs_"] = "Counter-Strike" -- Hostage Rescue
...
-- Hidden maps
--
local IgnorePatterns = {"^background", "^devtest", "^ep1_background", "^ep2_background", "^styleguide", "^sdk_", "^test_", "^vst_",}
local IgnoreMaps = {
  c4a1y = true,
  credits = true,
  d2_coast_02 = true,
...
local function IsUselessMap(map_name)
  ...
  for _, ignore in ipairs(IgnorePatterns) do
    if string.find(map_name, ignore) then return true end
  end
  return false
end

local function AddMapInfo(map_info, cat, cat_name, cat_table)
  if not cat_table[cat] then
    cat_table[cat] = {
      name = cat_name,
      maps = {}
    }
...
RefreshMaps = function(skip)
  if not skip then UpdateGamemodeMaps() end
  MapList = {}
  GameMapList = {}
  local ExistingMaps = {}
  ...
  for id, tab in pairs(games) do
    if not tab.mounted then continue end
    ...
    for k, v in ipairs(maps) do
      ...
      if not Category then
        ...
        for pattern, category in pairs(patterns) do
          if string.find(map_name, pattern) then Category = category end
        end
      end
      ...
      -- Favourite maps
      if IsMapFavourite(map_name) then AddMapInfo(map_info, "Favourites", "Favourites", MapList) end
...
function GetMapCategories(catType)
  ...
  if catType == "game" then
    for cat, tab in pairs(GameMapList) do
      if not tab or not tab.maps or #tab.maps < 1 then continue end
      output[cat] = tab.name
    end
  else
...
local function map_cat_helper(map, search_t)
  for cat, tab in pairs(search_t) do
    ...
    for _, map_t in pairs(tab.maps) do
      if map_t.name == map:lower() then return cat end
    end
  end
end
...
function GetMapCategory(map)
  ...
  if not r then r = map_cat_helper(map, GameMapList) end
  return r
end

function GetMapsFromCategory(cat)
  if not DoesCategoryExist(cat) then return {} end
...
function DoesCategoryExist(cat)
  if not MapList[cat] and not GameMapList[tonumber(cat)] then return false end
  return true
end
