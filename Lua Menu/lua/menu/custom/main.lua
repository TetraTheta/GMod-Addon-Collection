...
function PANEL:Paint()
  if self.Disabled == true then
    self:SetFGColor(Color(120, 120, 120))
    return
  end
...
function PANEL:Init()
  ...
  Achievements.DoClick = function() self:GetParent():OpenAchievementsMenu() end
  local Options = vgui.Create("MenuButton", mainButtons)
  Options:Dock(TOP)
  Options:SetText("#options")
  Options:DockMargin(5, 20, 5, 20)
  Options.DoClick = function() RunGameUICommand("OpenOptionsDialog") end
  local Disconnect = vgui.Create("MenuButton", mainButtons)
  Disconnect:Dock(TOP)
  Disconnect:SetText("#disconnect")
  Disconnect:DockMargin(5, 5, 5, 0)
  Disconnect.DoClick = function() RunGameUICommand("Disconnect") end
  self.Disconnect = Disconnect
  local Quit = vgui.Create("MenuButton", mainButtons)
  Quit:Dock(TOP)
  Quit:SetText("#quit")
  Quit:DockMargin(5, 0, 5, 0)
  Quit.DoClick = function() RunGameUICommand("quit") end
end

local old = 0
function PANEL:Paint()
  if ScrH() ~= old then old = ScrH() end
  if not self.Image or self.Image:GetName() ~= "../gamemodes/" .. engine.ActiveGamemode() .. "/logo" then self.Image = Material("../gamemodes/" .. engine.ActiveGamemode() .. "/logo.png", "nocull smooth") end
  if self.Image and not self.Image:IsError() then
