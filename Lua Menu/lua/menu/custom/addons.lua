...
function PANEL:OnMouseReleased(mousecode)
  if mousecode == MOUSE_RIGHT then
    local m = DermaMenu()
    if not self.panel.ToggleMounted:GetDisabled() then
    ...
    if steamworks.ShouldMountAddon(self.Addon.wsid) then
      m:AddOption("Disable", function()
        steamworks.SetShouldMountAddon(self.Addon.wsid, false)
        steamworks.ApplyAddons()
      end)
    else
      m:AddOption("Enable", function()
        steamworks.SetShouldMountAddon(self.Addon.wsid, true)
        steamworks.ApplyAddons()
      end)
    end

    m:AddOption("Uninstall", function()
      steamworks.Unsubscribe(self.Addon.wsid) -- Do we need ApplyAddons here?
      steamworks.ApplyAddons()
    end)
...
function PANEL:SetAddon(data)
  ...
  steamworks.FileInfo(data.wsid, function(result)
    ...
    if not file.Exists("cache/workshop/" .. result.previewid .. ".cache", "MOD") then steamworks.Download(result.previewid, false, function(name) end) end
...
local AddonFilters = {
  none = {
    label = "None",
    func = function(mod) return true end
  },
  enabled = {
    label = "Enabled",
    func = function(mod) return mod.mounted end
  },
  disabled = {
    label = "Disabled",
    func = function(mod) return not mod.mounted end
  },
}
...
local Grouping = {
  ...
  ptags = {
    ...
    func = function(addons)
      ...
      for _, addon in pairs(engine.GetAddons()) do
        if not gDataTable[addon.wsid] then
          ...
          for _, tag in pairs(tags) do
            if tag == "Addon" then -- Don't duplicate ALL addons
              continue
            end

            if not Ptags[tag:lower()] then tag = "Other" end
...
function PANEL:Init()
  ...
  for id, f in pairs(AddonFilters) do
    Filters:AddChoice("Filter by: " .. f.label, id, not Filters:GetSelectedID())
  end
...
function PANEL:Think()
  ...
  for id, pnl in pairs(self.AddonList:GetChildren()) do
    ...
    if pnl.GetSelected and not pnl:GetSelected() then allSelected = false end
    if pnl.Addon and not steamworks.ShouldMountAddon(pnl.Addon.wsid) then onlyEnabled = false end
...
function PANEL:ToggleSelected()
  for id, pnl in pairs(self.AddonList:GetChildren()) do
    if not pnl.GetSelected or not pnl:GetSelected() then continue end
    steamworks.SetShouldMountAddon(pnl.Addon.wsid, not steamworks.ShouldMountAddon(pnl.Addon.wsid))
  end
...
function PANEL:InvertSelection()
  for id, pnl in pairs(self.AddonList:GetChildren()) do
    if not pnl.GetSelected then continue end
    pnl:SetSelected(not pnl:GetSelected())
  end
end
...
function PANEL:RefreshAddons()
  ...
  for id, group in SortedPairsByMemberValue(addons, "title") do
    ...
    for k, mod in pairs(group.addons) do
      if not AddonFilters[filter].func(mod) then continue end
