...
local function EnableMouseScroll(s)
  ...
  if not mousePressed then
    s.start = nil
    s.EnableMouseScrollEnabled = false
    return
  end

  if not s.start and s:IsChildHovered() and not s:GetVBar():IsChildHovered() and not s.EnableMouseScrollEnabled then
...
function GetMapsFromCategorySearch(cat, searchText)
  ...
  if not maps or #maps < 1 then return {} end
  local output = {}
  for _, map in SortedPairs(maps) do
    if searchText and not map.name:find(searchText:lower()) then continue end
...
function PANEL:Init()
  ...
  SingleMultiPlayer.SetValue = function(s, n)
    s.PlayerCount = tonumber(n)
    RunConsoleCommand("cl_maxplayers", n)
    s:DoUpdateText()
  end
  ...
  SingleMultiPlayer.DoUpdatePanels = function(s, parent)
    ...
    for id, pnl in pairs(parent:GetChildren()) do
      ...
      if not pnl.OldHeight then pnl.OldHeight = pnl:GetTall() end
  ...
  hook.Add("VGUIMousePressed", "NewGameMenu_FuyckingHack", function(pnl, mc)
    if not IsValid(SingleMultiPlayer) or not SingleMultiPlayer.Butts or vgui.GetHoveredPanel() == SingleMultiPlayer then return end
    for id, p in pairs(SingleMultiPlayer.Butts) do
      if vgui.GetHoveredPanel() == p then return end
    end
  ...
  SingleMultiPlayer.DoClick = function(s)
    ...
    for id, v in pairs({1, 2, 4, 8, 16, 32, 64}) do
      ...
      but.DoClick = function(buttt)
        s:SetValue(buttt.Value)
        s:CloseDropDown()
      end
  ...
  p2p_enabled.OnValueChanged = function(pnl, checked) if checked then sv_lan:SetChecked(false) end end
  ...
  StartGame.DoClick = function() self:LoadMap() end
...
function PANEL:ServerSettings_AddTextEntry(v, parent)
  ...
  if v.singleplayer then
    DTextEntry.Singleplayer = true
  else
    DTextEntry.Singleplayer = false
  end
  return DTextEntry
end
...
function PANEL:ServerSettings_AddCheckbox(v, parent)
  ...
  if v.singleplayer then
    CheckBox.Singleplayer = true
  else
    CheckBox.Singleplayer = false
  end
  return CheckBox
end
...
function PANEL:ServerSettings_AddSlider(v, parent)
  ...
  if v.singleplayer then
    Slider.Singleplayer = true
  else
    Slider.Singleplayer = false
  end
  return Slider
end
...
function PANEL:UpdateLanguage()
  ...
  for id, t in pairs(LocalizedShit) do
    if not IsValid(t.panel) then -- Not too sure about the removal of the element inside the loop. Is Lua OK with this?
      table.remove(LocalizedShit, id)
      continue
    end
...
function PANEL:Update()
  ...
  for cat, name in SortedPairsByValue(GetMapCategories()) do
    if cat == "Favourites" then -- We have custom handling of this category
      continue
    end
...
function PANEL:CacheIcon(map)
  if not self.IconListCache then self:BuildIconList() end
  ...
  if not self.IconListCache[map] then return matNoIcon end
...
function PANEL:SelectCat(cat)
  ...
  for _, map in SortedPairs(maps) do
    ...
    if not subCategories[map.name] then
  ...
  for cat_name, cat_maps in SortedPairs(categories) do
    ...
    for _, map in SortedPairsByMemberValue(cat_maps, "name") do
      ...
      if map.incompatible then
        ...
        if not gMapIcons[map.name] then gMapIcons[map.name] = self:CacheIcon(map.name) end
        button.m_Image:SetMaterial(gMapIcons[map.name])
      end

      button:SetSize(128, 128)
      button.DoClick = function() self:SelectMap(map.name) end
      ...
      button.PaintOver = function(pnl, w, h)
        if pnl:GetText() == self.CurrentMap then boxHover(0, 0, w, h, color_white) end
