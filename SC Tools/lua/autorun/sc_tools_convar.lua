...
if not ConVarExists(scAutoGodNPC) then
  CreateConVar(scAutoGodNPC, "0", flags, "Automatically make NPC in campaign maps invulnerable.", 0, 1)
end
...
if not ConVarExists(scAutoGodSAdmin) then
  CreateConVar(scAutoGodSAdmin, "0", flags, "Automatically make users in 'superadmin' user group invulnerable.", 0, 1)
end
...
if not ConVarExists(scConnectionMessage) then
  CreateConVar(scConnectionMessage, "0", flags, "Show connection message to chat.", 0, 1)
end
...
if not ConVarExists(scFlashlightAuto) then
  CreateConVar(scFlashlightAuto, "0", flags, "Automatically enable flashlight to players. 0 = Disable, 1 = Super Admin Only, 2 = All Players.", 0, 2)
end
...
if not ConVarExists(scIgnoreImportant) then
  CreateConVar(scIgnoreImportant, "0", flags, "Ignore 'important' flag of NPC", 0, 1)
end
...
if not ConVarExists(scNoPlayerCollision) then
  CreateConVar(scNoPlayerCollision, "0", flags, "Disable player-to-player collision.", 0, 1)
end
...
if not ConVarExists(scRemoveEffect) then
  CreateConVar(scRemoveEffect, "0", flags, "Entity remove effect type. 0 = Remove, 1 = Dissolve.", 0, 1)
end
