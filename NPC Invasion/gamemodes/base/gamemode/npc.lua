-- Based on 7b820815519971173f30ce487b71f6c8f48744b7
-- TODO: Fix this
...
function GM:GetDeathNoticeEntityName(ent)
  ...
  if ent:IsVehicle() and ent.VehicleTable and ent.VehicleTable.Name then return ent.VehicleTable.Name end
  if ent:IsNPC() and ent.NPCTable and ent.NPCTable.Name then return ent.NPCTable.Name end
...
function GM:OnNPCKilled(ent, attacker, inflictor)
  ...
  if IsValid(attacker) then
    ...
    if not IsValid(inflictor) then
      InflictorClass = attacker:GetClass()
    end

    if attacker:IsPlayer() then
      local flags = 0
      if ent:IsNPC() and ent:Disposition(attacker) ~= D_HT then
        flags = flags + DEATH_NOTICE_FRIENDLY_VICTIM
      end

      -- SC start - Check attacker and inflictor
      if attacker:GetNWBool("ZINV_RemoveCorpse", false) == true then
        AttackerClass = "sc_" .. attacker:GetClass()
      end

      if ent:GetNWBool("ZINV_RemoveCorpse", false) == true then
        InflictorClass = "sc_" .. InflictorClass
      end
  ...
  -- It was NPC suicide..
  if ent == AttackerClass then
    InflictorClass = "suicide"
  end

  local flags = 0
  if IsValid(Entity(1)) and ent:IsNPC() and ent:Disposition(Entity(1)) ~= D_HT then
    flags = flags + DEATH_NOTICE_FRIENDLY_VICTIM
  end

  if IsValid(Entity(1)) and AttackerClass:IsNPC() and AttackerClass:Disposition(Entity(1)) ~= D_HT then
    flags = flags + DEATH_NOTICE_FRIENDLY_ATTACKER
  end

  -- SC start - check inflictor (NPC kill)
  if ent:GetNWBool("ZINV_RemoveCorpse", false) == true then
    InflictorClass = "sc_" .. InflictorClass
  end
