...
SWEP.Secondary.RPGRocket.Speed = 5000
...
-- RPG Rocket
-- https://github.com/ValveSoftware/source-sdk-2013/blob/master/mp/src/game/shared/hl2mp/weapon_rpg.cpp
function SWEP:SecondaryAttackRPGRocket()
  if SERVER then
    ...
    round:SetAngles(self:GetOwner():GetAimVector():Angle())
    round:SetSolidFlags(FSOLID_NOT_SOLID)
    round:Spawn()
    round:SetSaveValue("m_flLastHomingSpeed", 5000)
    round:SetSaveValue("m_flGracePeriodEndsAt", CurTime())
    -- round:SetAbsVelocity(self:GetOwner():GetAimVector() * 500)
    round:SetVelocity(self:GetOwner():GetAimVector() * 5000)
    timer.Simple(1, function()
      if IsValid(round) then
        round:SetSolidFlags(0)
      end
    end)
  end
